...
int main()
{
    ...
    {
        sprintf(fname1, "vel_x-y_t=%05d.dat", number);
            ...
            for (y = min_y; y < (max_y + eps); y += delta_y)
            {
                 ...
                 for (i = 0; i < N; i++)
                {
                    ...
                    if (data_bound[i][1] > x && data_bound[i][1] < x + delta_x && data_bound[i][2] > y && data_bound[i][2] < y + delta_y)
                    {
                        ...
                        sigma_pre += (data_bound[i][6] - v_z_bin) * (data_bound[i][6] - v_z_bin);
                ...
                // fprintf(fpwrite1, "%f %f %f\n", x, y, log10((count + eps)*1000/(delta_x*delta_y)));
                // count_tot += count;
                sigma = sqrt(sigma_pre / (count + eps));
                fprintf(fpwrite1, "%f %f %f\n", x, y, sigma*20.74);
                }
